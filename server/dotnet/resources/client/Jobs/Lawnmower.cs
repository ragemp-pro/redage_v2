...
namespace NeptuneEvo.Jobs
{
    class Lawnmower : Script
    {
        ...
        public void onPlayerEnterVehicleHandler(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                if (NAPI.Data.GetEntityData(vehicle, "TYPE") != "MOWER" || player.VehicleSeat != 0) return;
        ...
        private static void mowerCheckpointEnterWay(ColShape shape, Player player)
        {
            try
            {
                ...
                var vehicle = player.Vehicle;
                if (NAPI.Data.GetEntityData(vehicle, "TYPE") != "MOWER" || Main.Players[player].WorkID != 5 || !player.GetData<bool>("ON_WORK") || player.GetData<int>("WORKWAY") != shape.GetData<int>("WAY") || player.GetData<int>("WORKCHECK") != shape.GetData<int>("NUMBER"))
                    return;
