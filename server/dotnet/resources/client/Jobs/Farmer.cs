...
namespace NeptuneEvo.Jobs.FarmerJob
{
    public class Farmer : Script
    {
        #region  Settings
        private static nLog Log = new nLog("Farmer Golem");
        private static List<CharacterData> Farmers = new List<CharacterData>();
        private static ColShape checkpoint;
        private static Random rnd = new Random();
        private static int minsec = 40;
        private static int maxsec = 100;
        private static int maxlvl = 25;
        #endregion
        ...
        public void Event_FarmerStart()
        {
            try
            {
                ...
                NAPI.Blip.CreateBlip(677, new Vector3(438.3, 6510.9, 22.4), 1, 24, "Ферма", 255, 0, true, 0, 0); // Блип на карте
                NAPI.TextLabel.CreateTextLabel("~g~Фермер Голем", new Vector3(438.3554, 6510.949, 29.6), 10f, 0.1f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension); // Над головой у Ped
                List<Vector3> shapes = new List<Vector3>()
                {
                    new Vector3(438.35, 6510.94, 28), //Голем
                };
                
                var golemShape = NAPI.ColShape.CreateCylinderColShape(shapes[0], 2.1f, 3, 0);
                ...
                Timers.Start($"{player.Name}farmer{colID}", 5000, () =>
                {
                    ...
                    if (player.GetData<int>($"regenplant{colID}") < 1) 
                    {
                        if (!player.GetData<bool>($"seedplant{colID}")) 
                        {
                            ...
                            NAPI.Task.Run(() => Timers.Stop($"{player.Name}farmer{colID}"));
                        }
                        else
                        {
                            player.ResetData($"regenplant{colID}");
        ...
        public static void SaveLvl(Player player, string workname)
        {
            try
            {
                ...
                if (result == null || result.Rows.Count == 0)
                {
                    ...
                    Log.Write("Пользователь внесен в базу", nLog.Type.Warn);
                }
                else
                {
