    ...
    class Commands
    {
        ...
        public static void InviteToFamily(Player sender, Player target)
        {
            try
            {
                ...
                Trigger.ClientEvent(target, "openDialog", "INVITEDTOFAMILY", $"{sender.Name} пригласил Вас в {Family.GetFamilyName(sender)}");
        ...
        public static void SetFamilyRank(Player sender, Player target, int newrank)
        {
            try
            {
                ...
                if (sender == target)
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не можете изменять свой же ранг", 3000);
                    return;
                }
                ...
                Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"Теперь ты {Manager.Members[target].FamilyRankName} во фракции", 3000);
