...
namespace NeptuneEvo.Fractions
{
    class CarDelivery : Script
    {
            ...
            try
            {
                    ...
                    colShape.OnEntityEnterColShape += (s, e) =>
                    {
                        try
                        {
                            if (!Main.Players.ContainsKey(e) || !e.IsInVehicle || !e.Vehicle.HasData("ACCESS")
                                || e.Vehicle.GetData<string>("ACCESS") != "GANGDELIVERY" || s.GetData<int>("ID") != e.Vehicle.GetData<int>("ENDPOINT")) return;
                ...
                foreach (var pos in MafiaEndDelivery)
                {
                    ...
                    colShape.OnEntityEnterColShape += (s, e) =>
                    {
                        ...
                        {
                            if (!Main.Players.ContainsKey(e) || !e.IsInVehicle || !e.Vehicle.HasData("ACCESS")
                                || e.Vehicle.GetData<string>("ACCESS") != "MAFIADELIVERY" || s.GetData<int>("ID") != e.Vehicle.GetData<int>("ENDPOINT")) return;
        ...
        public void Event_PlayerEnterVehicle(Player player, Vehicle vehicle, sbyte seat)
        {
            try
            {
        ...
        public static void Event_PlayerDisconnected(Player player)
        {
            ...
            {
                Vehicle veh = player.GetData<Vehicle>("DELIVERY_CAR");
        ...
        [RemoteEvent("gangmis")]
        public void Event_gangMission(Player player, int id)
        {
            try
            {
                if (!Main.Players.ContainsKey(player) || player.IsInVehicle) return;
        ...
        public void Event_mafiaMission(Player player, int id)
        {
            try
            {
                if (!Main.Players.ContainsKey(player) || player.IsInVehicle) return;
                int i = player.GetData<int>("ONMAFIAID");
