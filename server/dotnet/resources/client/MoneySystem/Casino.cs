...
namespace NeptuneEvo.MoneySystem
{
    class Casino : Script
    {
        ...
        private static string blipName = config.TryGet<string>("BlipName", "Казино");
                ...
                { // Если ставки были закрыты
                    ...
                    { 
                        if (type >= 1)
                        {
                            ...
                            GameLog.Money("Казино", $"player({Main.Players[player].UUID})", (cred + czero + cblack), $"type{type}");
                        }
                    }
                }
        ...
        public static void Interact(Player client)
        {
        ...
        public static void Disconnect(Player client, DisconnectionType type)
        {
                            ...
                            foreach (Player target in PlayersList.Keys)
                            {
                                NAPI.Task.Run(() =>
                                {
                                    try
                                    {
                                        ...
                                        GameLog.Money($"player({Main.Players[target].UUID})", "Казино", (cred + czero + cblack), "placedBet");
                                    } catch(Exception e)
                                    {
                        ...
                        try
                        {
                            ...
                            else if (index == 1)
                            {
                                ...
                                if (wonbet >= 1)
                                {
                                    wonbet = wonbet * 2;
                                    Wallet.Change(target, wonbet);
                                    GameLog.Money("Казино", $"player({Main.Players[target].UUID})", wonbet, "winRed");
                                }
                            }
                            else if (index == 2)
                            {
